class AllowNonconsecutiveSubtask < ActiveRecord::Migration
  def up
    add_column :testdata_sets, :td_list, :string, :null => false
    add_column :testdata_sets, :constraints, :text
    TestdataSet.update_all('`td_list` = IF(`from` = `to`, `from`, CONCAT(`from`, "-", `to`))')
  ...
  def down
    ...
    TestdataSet.update_all('`from` = SUBSTRING_INDEX(`td_list`, "-", 1), `to` = SUBSTRING_INDEX(`td_list`, "-", -1)')
